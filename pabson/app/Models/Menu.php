...
class Menu extends Model
{
    ...
    public function getActiveClassAttribute()
    {
        if (in_array(request()->route()->getName(), explode(',', $this->related_routes))){
            return ($this->group_id == MenuGroupConstant::FRONT_MENU_ID || $this->group_id == MenuGroupConstant::FRONT_MOBILE_MENU_ID) ? 'active' : 'kt-menu__item--open';
        }
        if(strtolower($this->route) == strtolower(request()->route()->getName()) ) {
            return ($this->group_id == MenuGroupConstant::FRONT_MENU_ID || $this->group_id == MenuGroupConstant::FRONT_MOBILE_MENU_ID) ? 'active' : 'kt-menu__item--active';
        }
        else {
            return null;
        }
